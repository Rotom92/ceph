...
void Objecter::list_nobjects(NListContext *list_context, Context *onfinish)
{
  ...
  // Are we operating over a split of the range, or the whole thing?
  const bool range_split = list_context->worker_m > 0;

  const uint32_t chunks_per_pg = list_context->worker_m;
  const uint32_t chunks_per_worker = osdmap->get_pg_pool(list_context->pool_id)->get_pg_num();

  uint32_t pg_min;
  uint32_t pg_max;
  if (range_split) {
    pg_min = (list_context->worker_n * chunks_per_worker) / chunks_per_pg;
    if (list_context->worker_n == list_context->worker_m - 1) {
      ...
      pg_max = (list_context->worker_n * chunks_per_worker + chunks_per_worker - 1) / chunks_per_pg;
    }
  } else {
    pg_min = 0;
    pg_max = list_context->starting_pg_num - 1;
  ...
  ldout(cct, 10) << "Worker " << list_context->worker_n << "/" << list_context->worker_m
    << " per-worker:" << chunks_per_worker << " per-pg:"
    << chunks_per_pg << dendl;

  ldout(cct, 10) << "Current PG " << list_context->current_pg << " min/max= "
    << pg_min << "/" << pg_max << dendl;

  if (range_split && list_context->object_hash_high == 0x0) {
    // Initial call
...
void Objecter::_nlist_recalc_limits(NListContext *list_context)
{
  ...
  const uint32_t chunks_per_pg = list_context->worker_m;
  const uint32_t chunks_per_worker = pg_num;

  ldout(cct, 10) << "Upper bit mask: 0x" << std::hex << local_hash_to_osd_hash(pg_num_mask) << std::dec << dendl;
  ...
  uint32_t hash_split_size = (hash_range_size / chunks_per_pg);

  uint32_t first_chunk = list_context->worker_n * chunks_per_worker;
  uint32_t this_pg_base_chunk = chunks_per_pg * list_context->current_pg;

  uint32_t last_chunk = list_context->worker_n * chunks_per_worker + chunks_per_worker - 1;
  if (list_context->worker_n == list_context->worker_m - 1) {
    last_chunk = chunks_per_worker * list_context->worker_m - 1;
  }
  uint32_t this_pg_last_chunk = chunks_per_pg * list_context->current_pg + chunks_per_pg - 1;
  ldout(cct, 10) << "This PG has chunks " << this_pg_base_chunk << " thru "
    << this_pg_last_chunk << dendl;

  bool is_first_chunk_in_pg = false;
  uint32_t hash_low_counter;
  if (first_chunk <= this_pg_base_chunk) {
    hash_low_counter = 0x0;
    is_first_chunk_in_pg = true;
  } else {
  ...
  bool is_last_chunk_in_pg = false;
  uint32_t hash_high_counter;
  if (last_chunk >= this_pg_last_chunk) {
    hash_high_counter = hash_range_size; // Top of the PG
    is_last_chunk_in_pg = true;
  } else {
    uint32_t this_pg_last_chunk = last_chunk - this_pg_base_chunk;
    hash_high_counter = (this_pg_last_chunk + 1) * hash_split_size - 1;
  }

  uint32_t object_hash_mask = local_hash_to_osd_hash(pg_num_mask);

  uint32_t pg_low_bits = local_hash_to_osd_hash(list_context->current_pg);
  uint32_t pg_high_bits = local_hash_to_osd_hash(list_context->current_pg);
  if (is_last_chunk_in_pg) {
  ...
  if (is_last_chunk_in_pg) {
    uint32_t hash_high_2 = (0xffffffff & (~object_hash_mask)) | (pg_high_bits & object_hash_mask);
    ldout(cct, 10) << "UPPER BOUND hash_high_2 = 0x" << std::hex << hash_high_2 << std::dec << dendl;
    assert(hash_high == hash_high_2);
  }
  if (is_first_chunk_in_pg) {
    uint32_t hash_low_2 = (0x00000000 & (~object_hash_mask)) | (pg_low_bits & object_hash_mask);
...
{
  ldout(cct, 10) << __func__ << dendl;
  ldout(cct, 10) << __func__ << " cookie: " << list_context->cookie << dendl;
  ...
  for (std::list<librados::ListObjectImpl>::iterator i = response.entries.begin();
    ...
    assert(local_hash_to_osd_hash(hash_pos) >= list_context->object_hash_low);
    if (local_hash_to_osd_hash(hash_pos) < list_context->object_hash_low) {
      lderr(cct) << "OSD returned unexpectedly low hash value 0x" << std::hex
        << local_hash_to_osd_hash(hash_pos) << " vs low bound 0x"
        << list_context->object_hash_low << std::dec << dendl;
      final_finish->complete(-EIO);
      return;
    }

    // Truncate entries at object_hash_high
    if (local_hash_to_osd_hash(hash_pos) > list_context->object_hash_high) {
      ldout(cct, 10) << "Truncating (0x" << std::hex << local_hash_to_osd_hash(hash_pos) << " > 0x"
  ...
  if (list_context->cookie.get_hash() > list_context->object_hash_high) {
    ldout(cct, 10) << "Cookie (0x" << std::hex
